# Nanobot VM - Ubuntu 24.04 with Python 3.11+, uv, and Nix
# Uses macvtap (direct mode) - VM gets IP on main network (192.168.1.x)
# SSH: ssh root@<VM_IP> or ssh muad@<VM_IP>
#
# Persistent config: Nanobot config directory ~/.nanobot is symlinked to /mnt/share/nanobot
...
# - Overlay: /mnt/arrakis/nanobot/disk/nanobot.qcow2 (created automatically on nixos-rebuild switch)
# - Update base: Change URL/hash in config, overlay auto-recreates
# - Reset to pristine: rm /mnt/arrakis/nanobot/disk/nanobot.qcow2 && systemctl restart libvirt-guest-nanobot
#
...
let
  ...
  userData = pkgs.writeText "user-data" ''
    ...
    ${lib.concatMapStrings (key: "          - ${key}\n") sshKeys}
        bootcmd:
          - mkdir -p /mnt/share
        packages: [qemu-guest-agent, ca-certificates, curl, gnupg, nfs-common, build-essential, git, python3, python3-pip, python3-venv]
        runcmd:
          - DEBIAN_FRONTEND=noninteractive apt-get update
          - DEBIAN_FRONTEND=noninteractive apt-get upgrade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"
          - DEBIAN_FRONTEND=noninteractive apt-get install -y nfs-common
          - mkdir -p /mnt/share
          - grep -q '^192.168.1.251:/mnt/arrakis/ix/nanobot/share[[:space:]]\+/mnt/share[[:space:]]\+nfs' /etc/fstab || printf '%s\n' '192.168.1.251:/mnt/arrakis/ix/nanobot/share /mnt/share nfs defaults,_netdev,nofail 0 0' >> /etc/fstab
          - mount -av || true
          # Wait for NFS mount to be ready, then fix ownership for muad
          - while ! mountpoint -q /mnt/share 2>/dev/null; do sleep 1; done
          - mkdir -p /mnt/share/nanobot
          - chown -R muad:muad /mnt/share/nanobot
          - systemctl enable --now qemu-guest-agent || true
          - sed -i 's/^#*PermitRootLogin.*/PermitRootLogin prohibit-password/' /etc/ssh/sshd_config
          - systemctl restart ssh
          # Install uv (Python package manager) and nanobot
          - curl -LsSf https://astral.sh/uv/install.sh | sh
          - export PATH="/root/.local/bin:$PATH"
          - cp /root/.local/bin/uv /usr/local/bin/uv
          - cp /root/.local/bin/uvx /usr/local/bin/uvx
          - sudo -H -u muad bash -c 'curl -LsSf https://astral.sh/uv/install.sh | sh'
          - sudo -H -u muad bash -c 'export PATH="$HOME/.local/bin:$PATH" && uv tool install nanobot-ai || (echo "nanobot install failed (continuing)" >&2)'
          - curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install --no-confirm
          # Create Nanobot persistent config directory on NFS share
          - mkdir -p /mnt/share/nanobot
          # Ensure Nanobot config is symlinked to persistent share for muad user
          - sudo -H -u muad bash -c 'if [ -e "$HOME/.nanobot" ] && [ ! -L "$HOME/.nanobot" ]; then mv "$HOME/.nanobot" "$HOME/.nanobot.backup.$(date +%s)"; fi'
          - sudo -H -u muad bash -c 'ln -sfn /mnt/share/nanobot "$HOME/.nanobot"'
  '';
